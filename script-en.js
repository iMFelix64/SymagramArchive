...
function prepareProjectDirectoryEnter({ force = false } = {}) {
  ...
  const bottomRuleDelay = PROJECT_DIRECTORY_LIST_DELAY_MS + Math.max(0, enterItems.length - 1) * 60;

  projectGroup.style.setProperty("--project-directory-rule-delay", `${PROJECT_DIRECTORY_LIST_DELAY_MS}ms`);
  projectGroup.style.setProperty("--project-directory-bottom-rule-delay", `${bottomRuleDelay}ms`);
